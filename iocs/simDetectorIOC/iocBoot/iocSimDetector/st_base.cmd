...
epicsEnvSet("ADCOMPVISION","$(AREA_DETECTOR)/ADCompVision")
epicsEnvSet("ADPLUGINBAR","$(AREA_DETECTOR)/ADPluginBar")
...
dbLoadRecords("$(NDPlUGINWARP)/db/NDWarp.template","P=$(PREFIX),R=warp1:,PORT=Warp1,ADDR=0,TIMEOUT=1,NDARRAY_PORT=$(PORT)")

set_requestfile_path("$(ADSIMDETECTOR)/simDetectorApp/Db")

# Access Security
dbLoadRecords("$(XXX)/xxxApp/Db/Security_Control.db","P=$(PREFIX)")
asSetFilename("$(XXX)/iocBoot/accessSecurity.acf")
...
dbl > pvnames.txt
dbgrep("$(PREFIX)image1:*") > image1pv.txt
dbgrep("$(PREFIX)cam1:*") > pvCam1.txt
